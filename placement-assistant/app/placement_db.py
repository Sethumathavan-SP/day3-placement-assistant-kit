...
class PlacementDb:
    ...
    def create_application(self, student_id: int, drive_id: int) -> tuple[int, bool]:
        ...
        cur = self.conn.execute("""INSERT INTO application (student_id, drive_id, created_at)
                                  VALUES (?, ?, ?) ON CONFLICT(student_id, drive_id) DO NOTHING""",
                                (student_id, drive_id, self.clock()))
        if cur.rowcount == 1:
            return cur.lastrowid, True
        return self.conn.execute("SELECT id FROM application WHERE student_id = ? AND drive_id = ?",
                                 (student_id, drive_id)).fetchone()[0], False

    def claim_slot(self, slot_id: int, student_id: int, expected_version: int) -> bool:
        """Book a free slot only when its version is unchanged."""
        cur = self.conn.execute("""UPDATE interview_slot SET student_id = ?, version = version + 1
    ...
    def record_notification(self, roll_no: str, message: str, dedupe_key: str) -> tuple[int, bool]:
        ...
        cur = self.conn.execute("""INSERT INTO notification (roll_no, message, dedupe_key, created_at)
