...
class RunStore:
    ...
    def claim_next(self, worker_id: str, lease_seconds: float) -> Claimed | None:
        ...
        now = self.clock()
        with self.transaction() as c:
            row = c.execute("""SELECT id, thread_id, attempts FROM run
            ...
            c.execute(f"""UPDATE run SET status = 'running', lease_owner = ?, lease_until = ?,
                         attempts = ?, started_at = COALESCE(started_at, {NOW_SQL}) WHERE id = ?""",
    ...
    def heartbeat(self, run_id: str, worker_id: str, lease_seconds: float) -> bool:
        ...
        with self.transaction() as c:
            cur = c.execute("""UPDATE run SET lease_until = ?
    ...
    def reap_expired(self) -> list[str]:
        ...
        with self.transaction() as c:
            ...
            touched = []
            for row in rows:
                status = "queued" if row["attempts"] < row["max_attempts"] else "dead"
                finished = f", finished_at = {NOW_SQL}" if status == "dead" else ""
                c.execute(f"""UPDATE run SET status = ?, available_at = ?, error_code = 'lease_expired',
                             lease_owner = NULL, lease_until = NULL{finished} WHERE id = ?""",
                          (status, now, row["id"]))
    ...
    def request_cancel(self, run_id: str) -> str | None:
        ...
        with self.transaction() as c:
            ...
            return c.execute("SELECT status FROM run WHERE id = ?", (run_id,)).fetchone()["status"]
    ...
    def mark_cancelled(self, run_id: str, worker_id: str) -> bool:
        ...
        with self.transaction() as c:
            cur = c.execute(f"""UPDATE run SET status = 'cancelled', lease_owner = NULL,
                         lease_until = NULL, finished_at = {NOW_SQL}
    ...
    def fail_attempt(self, run_id: str, worker_id: str, error_code: str, retryable: bool,
        ...
        with self.transaction() as c:
            ...
            max_attempts = c.execute("SELECT max_attempts FROM run WHERE id = ?", (run_id,)).fetchone()[0]
            if not retryable:
                status, available = "failed", None
            elif attempts < max_attempts:
                status = "queued"
                available = self.clock() + backoff_seconds * (2 ** (attempts - 1))
            else:
                status, available = "dead", None
            finished_sql = f", finished_at = {NOW_SQL}" if status != "queued" else ""
            c.execute(f"""UPDATE run SET status = ?, error_code = ?, available_at = COALESCE(?, available_at),
                         lease_owner = NULL, lease_until = NULL{finished_sql} WHERE id = ?""",
                      (status, error_code, available, run_id))
            return status
